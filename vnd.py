...
class VND():
	...
	def run(self):
		...
		# Variável que controla se houve melhoria
		improvement = True
		...
		while improvement:
			...
			while k <= MAX_K:
				...
				if k == 1:
					# Solução f(s')
					f_sl = self.n_s1(s).calculate_number_colors()

				# Estrutura de vizinhança 2
				elif k == 2:
					f_sl = self.n_s2(s).calculate_number_colors()
				
				# Estrutura de vizinhança 3
				else:
					# Solução f(s')
					f_sl = self.n_s3(s).calculate_number_colors()
				...
				if self.check_improvement(f_sl, f_s):
					...
					# Atualizando melhoria
					improvement = True

				# Senão passa para a próxima estrutura de vizinhança
				else:
					k = k + 1
